...
class ObsClient
{
    ...
    public function __construct(array $config = [], bool $enableCoroutineHandler = false)
    {
        $this->factorys = [];
        ...
        if ($enableCoroutineHandler && class_exists(\Hyperf\Guzzle\CoroutineHandler::class)) {
            $handlerStack = HandlerStack::create(new \Hyperf\Guzzle\CoroutineHandler());
        } else {
     ...
     * @param $enableCoroutineHandler
     * @return ObsClient
     */
    public static function factory(array $config = [], bool $enableCoroutineHandler = false)
    {
        return new ObsClient($config, $enableCoroutineHandler);
    }
